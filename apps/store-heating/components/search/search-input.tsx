...
/**
 * Client component. The single search-input field used by both the search
 * page (`variant="page"`) and the header overlay (`variant="overlay"`).
 *
 * Behaviour:
 *   - Typing fires a debounced fetch to /api/predictive-search.
 *   - Pressing Enter (or clicking Submit) navigates to /[locale]/search?q=…
 *   - Predictive results render below the input as a clickable list — only
 *     when the variant is "overlay" (the search page already shows full
 *     product cards in the result grid).
 *
 * Keyboard:
 *   - Esc clears the field (overlay variant calls onClose if provided).
 *   - Up/Down arrows move focus through the predictive list.
 */
...
export function SearchInput({
  ...
  const showResults = variant === "overlay" && predictive && (predictive.products.length > 0 || predictive.queries.length > 0);

  return (
    <div className={variant === "overlay" ? "w-full" : "w-full"}>
      <form
        role="search"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      {variant === "overlay" ? (
        <div className="mt-6 min-h-[6rem]">
                ...
                <div>
                  ...
                  <ul className="mt-3 grid grid-cols-2 gap-4">
                    {predictive.products.map((p) => (
                      <li key={p.id}>
                        <Link
                          href={localeHref(locale, `/products/${p.handle}`)}
                          onClick={onClose}
          ...
          {predictive && predictive.products.length === 0 && predictive.queries.length === 0 ? (
