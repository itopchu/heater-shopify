 ...
 *
 * Phone + (German) prefilled message come from `lib/gberg/contact.ts` so a
 * single edit there updates every WhatsApp entry point. Per requirements §2
 * the message body stays German across all storefront locales — only the
 * aria-label localizes.
 */
import {useT} from '~/lib/gberg/i18n';
import {whatsappHref, WHATSAPP_MESSAGE_GENERIC} from '~/lib/gberg/contact';

export function WhatsAppBubble() {
  const t = useT();
  const href = whatsappHref(WHATSAPP_MESSAGE_GENERIC);
  ...
  return (
    <a
      ...
      className="whatsapp-bubble fixed bottom-3 right-3 z-[60] flex h-11 w-11 items-center justify-center rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.18),0_2px_6px_rgba(0,0,0,0.12)] transition-transform duration-200 ease-out hover:scale-[1.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#25D366] md:bottom-6 md:right-6 md:h-14 md:w-14"
